...
scores_df <- read.csv("scores.csv", stringsAsFactors = TRUE)
summary(scores_df)
...
voter_turnout_df2 <- read_excel("G04ResultsDetail2004-11-02.xls", sheet = "Voter Turnout", skip = 2, col_names = c("ward_precint", "ballots_cast", "registered_voters", "voter_turnout"))
...
db <- dbConnect(drv = RSQLite::SQLite(), dbname = "example.db")
...
person_df <- dbGetQuery(conn = db, statement = "SELECT * FROM PERSON;")
head(person_df)
